...
class MyEComplaintRecordVC: UIViewController {
    var currentTabIndex = 0
    @IBOutlet weak var complaintContainerView: UIView!
    ...
    @objc func NotificationAction(_ notification: NSNotification) {
        if let userInfo = notification.userInfo {
            if let index = userInfo["userInfo"] as? Int {
                ...
                switch index {
                case 1:
                    currentVc = AppStrings.ViewControllerIdentifiers.myComplaintPendingRecordVc.getIdentifier
                case 2:
                    currentVc = AppStrings.ViewControllerIdentifiers.myComplaintResolvedRecordvc.getIdentifier
                default:
                ...
                if !currentVc.isEmpty {
                    ...
                    embed(viewController: vc, inView: complaintContainerView)
                }
            }
        }
    }
    func complaintPage(type:ScreenType) {
        switch type {
        case .Pending:
    ...
    func tapGestureRecognizers() {
        pandingRecordView.addTapGestureRecognizer {
            self.complaintPage(type: .Pending)
        }
        resolvedRecordView.addTapGestureRecognizer {
            self.complaintPage(type: .Resolved)
        }
    }
}
